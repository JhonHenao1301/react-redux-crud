...
export default function Navbar() {
    ...
    return (
        <nav className="dark:bg-slate-900 dark:opacity-90 dark:text-slate-100 bg-stone-200 opacity-80 w-full sticky top-0 left-0 z-20 shadow-sm text-slate-950">
                    ...
                    <ul className={`navbar-ul ${open ? 'top-20 ': 'top-[-490px]'}`}>
                        {[
                            ...
                            <li className="navbar-li" key={index}>
                                <a href={link} className="navbar-link dark:after:bg-slate-200 rounded-lg font-medium duration-500" onClick={()=>setOpen(!open)}>
                                    {name}
                                </a>
                            </li>
                        ))}
